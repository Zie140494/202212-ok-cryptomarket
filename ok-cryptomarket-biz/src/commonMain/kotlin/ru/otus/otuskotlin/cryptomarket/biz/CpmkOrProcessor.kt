...
class CpmkOrProcessor(private val settings: CpmkCorSettings = CpmkCorSettings()) {
    ...
    companion object {
        private val BusinessChain = rootChain<CpmkContext> {
            ...
            operation("Создание объявления", CpmkCommand.CREATE) {
                stubs("Обработка стабов") {
                    stubCreateSuccess("Имитация успешной обработки")
                    stubValidationBadWalletNumber("Имитация ошибки валидации заголовка")
                    stubValidationBadAccountNumber("Имитация ошибки валидации описания")
                ...
                validation {
                    ...
                    finishOrValidation("Завершение проверок")
                }
                chain {
            ...
            operation("Получить ордер", CpmkCommand.READ) {
                ...
                validation {
                    ...
                    finishOrValidation("Успешное завершение процедуры валидации")
                }
                computeOrState("Вычисление состояния объявления")
                chain {
                    title = "Логика чтения"
                    repoRead("Чтение ордера из БД")
                    worker {
            ...
            operation("Изменить объявление", CpmkCommand.UPDATE) {
                stubs("Обработка стабов") {
                ...
                validation {
                    worker("Копируем поля в adValidating") { orValidating = orRequest.deepCopy() }
                    worker("Очистка id") { orValidating.id = CpmkOrId(orValidating.id.asString().trim()) }
                ...
                chain {
                    ...
                    repoUpdate("Обновление ордера в БД")
                }
                prepareResult("Подготовка ответа")
            }
            operation("Удалить ордера", CpmkCommand.DELETE) {
                stubs("Обработка стабов") {
                ...
                chain {
                    ...
                    repoDelete("Удаление ордера из БД")
                }
                prepareResult("Подготовка ответа")
            }
            operation("Поиск ордеров", CpmkCommand.SEARCH) {
                stubs("Обработка стабов") {
                ...
                validation {
                    ...
                    finishOrFilterValidation("Успешное завершение процедуры валидации")
                }
